...
class Player(BasePlayer):
    ...
    game_type = models.StringField(initial='pCG')
...
def calc_total_payoff(player: Player):
    ...
    chance_to_win = (player.total_score + 1200)/4000
    player.win_bonus = chance_to_win > random.uniform(0, 1)
    bonus = 0
    if player.win_bonus:
        bonus = C.BONUS_FEE
...
class EndOfExperiment(Page):
    ...
    @staticmethod
    def vars_for_template(player: Player):
        calc_total_payoff(player)
        return dict()
page_sequence = [InformedConsentPage, Introduction, Decision, ResultsWaitPage, Results, EndOfSuperGame, ReadMe, EndOfExperiment]